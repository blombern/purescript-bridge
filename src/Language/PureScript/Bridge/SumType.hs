...
mkSumType p = SumType (mkTypeInfo p) constructors is
  where
    constructors = gToConstructors (from (undefined :: t))
    is = [Generic] <> maybeToList (nootype constructors) <> [EncodeJson, DecodeJson]

-- | Purescript typeclass instances that can be generated for your Haskell types.
data Instance = Generic | Newtype | DecodeJson | EncodeJson | Eq | Ord deriving (Eq, Show)
